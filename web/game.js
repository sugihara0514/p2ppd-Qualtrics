...
export function startGame(channel, rtc, opts = {}) {
    ...
    block.innerHTML = `
      <div id="precheck_panel" role="group" aria-labelledby="precheck_title">
        ...
        <label class="precheck_item">
          ...
          <span class="precheck_mark" aria-hidden="true"></span>
          <span class="precheck_text">自分の映像が表示されている</span>
        </label>
        <label class="precheck_item">
          <input type="checkbox" id="precheck_remote_video">
          <span class="precheck_mark" aria-hidden="true"></span>
          <span class="precheck_text">相手の映像が表示されている</span>
        </label>
        <label class="precheck_item">
          <input type="checkbox" id="precheck_audio">
          <span class="precheck_mark" aria-hidden="true"></span>
          <span class="precheck_text">相手の声が聞こえ、自分の声も届いている</span>
        </label>
        <label class="precheck_item">
          <input type="checkbox" id="precheck_ready">
          <span class="precheck_mark" aria-hidden="true"></span>
          <span class="precheck_text">問題がないのでゲームを開始できる</span>
        </label>
      </div>
    `;
  ...
  function setLayout(mode /* "emopred" | "choice" | "precheck" */) {
    ...
    if (mode === "emopred") {
      ...
      moveBlocksTo(layoutEmoPred, [blockMatrix, blockEmo, blockChoice]);
  ...
  let waitingChoiceResult = false; // 自分は送信済みで、相手待ち中
  ...
  async function submitChoice() {
    ...
    setChoiceButtonsEnabled(false);
    setChoiceWaitingUI(true);

    // choiceUI は消さない。見せたまま固定する
    showBase(choiceUI);
    moveIn(choiceUI);
 ...
 async function pollOnce() {
      try {
        ...
        if (serverStage === "choice") {
          ...
          if (!hasChosenThisRound && !canChoose && !waitingChoiceResult) {
  ...
  function setChoiceWaitingUI(on) {
    ...
    choiceUI.classList.toggle("disable", !!on);
